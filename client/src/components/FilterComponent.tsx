...
export const FilterComponent = () => {
    ...
    const columnsEmpty = !columnsBucket || columnsBucket.length === 0;
    ...
    return (
        <div className="">
            ...
            <Accordion type="single" collapsible className="w-full border rounded-md bg-white">
                {columnsBucket?.map((col) => (
                    <AccordionItem key={col.id} value={col.id} className="border-b">
                        <AccordionTrigger
                            className="flex flex-row items-center gap-2 px-3 py-2 hover:no-underline hover:bg-muted rounded-none cursor-pointer"
                            onClick={() => handleFilterColumnClick(col)}
                        >
                            <div className="flex flex-row items-center gap-2">
                                {col.prefixNode && (
                                    <span className="text-muted-foreground text-sm">
                                        <DraftingCompassIcon className="w-3 h-3" />
                                    </span>
                                )}
